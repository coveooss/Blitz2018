...
std::string serializeMapSize(const blitz::Map & map) {
...
std::string Networking::serializeProductions(const blitz::Map & map) {
    std::string returnString = "";
    std::ostringstream oss;
...
std::string Networking::serializeMap(const blitz::Map & map) {
    std::string returnString = "";
    std::ostringstream oss;
...
std::map<blitz::Location, unsigned char> Networking::deserializeMoveSet(std::string & inputString, const blitz::Map & m) {
    std::map<blitz::Location, unsigned char> moves = std::map<blitz::Location, unsigned char>();
...
void Networking::startAndConnectBot(std::string command) {
    ...
    //C:/xampp/htdocs/Blitz2k18/Blitz2k18/Debug/ExampleBot.exe
    //C:/Users/Michael/Anaconda3/python.exe
    //C:/Program Files/Java/jre7/bin/java.exe -cp C:/xampp/htdocs/Blitz2k18/AIResources/Java MyBot
...
int Networking::handleInitNetworking(unsigned char playerTag, const blitz::Map & m, bool ignoreTimeout, std::string * playerName) {
...
int Networking::handleFrameNetworking(unsigned char playerTag, const unsigned short & turnNumber, const blitz::Map & m, bool ignoreTimeout, std::map<blitz::Location, unsigned char> * moves) {
	...
	std::string productionString = serializeProductions(m);
	sendString(playerTag, productionString);
        sendString(playerTag, mapString);
        moves->clear();
        ...
        *moves = std::map<blitz::Location, unsigned char>();
    }
    catch(...) {
        if(response.empty()) player_logs[playerTag - 1] += "\nERRORED!\nNo response received.";
        else player_logs[playerTag - 1] += "\nERRORED!\nResponse received (if any):\n" + response;
