...
private:
    ...
    unsigned short fog_of_war;
    bool ignore_timeout;
    blitz::Map game_map;
    std::vector<std::string> player_names;
    std::vector< std::map<blitz::Location, unsigned char> > player_moves;
    ...
    //Full game
    std::vector<blitz::Map> full_frames; //All the maps!
...
public:
    Blitz2k18(unsigned short width_, unsigned short height_, unsigned short fogOfWar_, unsigned int seed_, unsigned short n_players_for_map_creation, Networking networking_, bool shouldIgnoreTimeout);
