...
std::vector<bool> Blitz2k18::processNextFrame(std::vector<bool> alive) {
    ...
    for(unsigned char a = 0; a < number_of_players; a++) {
        if(alive[a]) {
            frameThreads[threadLocation] = std::async(&Networking::handleFrameNetworking, &networking, a + 1, turn_number, filterMap(game_map, fog_of_war, a+1), ignore_timeout, &player_moves[a]);
            threadLocation++;
        }
    }
    ...
    std::vector< std::map<blitz::Location, unsigned char> > pieces(number_of_players);
    ...
    for(unsigned char a = 0; a < number_of_players; a++) if(alive[a]) {
        ...
        for(auto b = player_moves[a].begin(); b != player_moves[a].end(); b++) if(game_map.inBounds(b->first) && game_map.getSite(b->first).owner == a + 1) {
            ...
            blitz::Location newLoc = game_map.getLocation(b->first, b->second);
            if(pieces[a].count(newLoc)) {
                ...
                pieces[a].insert(std::pair<blitz::Location, unsigned char>(newLoc, game_map.getSite(b->first).strength));
            }

            //Add in a new piece with a strength of 0 if necessary.
            if(!pieces[a].count(b->first)) {
                pieces[a].insert(std::pair<blitz::Location, unsigned char>(b->first, 0));
            }
    ...
    for(unsigned short a = 0; a < game_map.map_height; a++) for(unsigned short b = 0; b < game_map.map_width; b++) {
        blitz::Site & s = game_map.contents[a][b];
        if(s.owner != 0) {
            blitz::Location l = { b, a };
            ...
            if(pieces[s.owner - 1].count(l)) {
                ...
                pieces[s.owner - 1].insert(std::pair<blitz::Location, unsigned char>(l, s.strength));
            }
            //Add to full production
    ...
    std::vector< std::map<blitz::Location, unsigned short> > toInjure(number_of_players);
        ...
        for(unsigned short c = 0; c < number_of_players; c++) if(alive[c] && pieces[c].count(l)) {
            for(unsigned short d = 0; d < number_of_players; d++) if(d != c && alive[d]) {
                ...
                if(pieces[d].count(tempLoc)) {
                    ...
                    else toInjure[d].insert(std::pair<blitz::Location, unsigned short>(tempLoc, pieces[c][l]));
                }
                //Check 'NORTH' square:
            ...
            if(game_map.getSite(l).strength > 0) {
                ...
                else toInjure[c].insert(std::pair<blitz::Location, unsigned short>(l, game_map.getSite(l).strength));
    ...
    //Add to full game:
    full_frames.push_back(blitz::Map(game_map));
...
//Public Functions -------------------
Blitz2k18::Blitz2k18(unsigned short width_, unsigned short height_, unsigned short fogOfWar_, unsigned int seed_, unsigned short n_players_for_map_creation, Networking networking_, bool shouldIgnoreTimeout) {
    ...
    fog_of_war = fogOfWar_;

    //Initialize map
    game_map = blitz::Map(width_, height_, n_players_for_map_creation, seed_);
    ...
    //Default initialize
    player_moves = std::vector< std::map<blitz::Location, unsigned char> >();
    turn_number = 0;
...
void Blitz2k18::output(std::string filename) {
...
GameStatistics Blitz2k18::runGame(std::vector<std::string> * names_, unsigned int seed, unsigned int id, bool enabledReplay, std::string replayDirectory) {
    //For rankings
    ...
    for(unsigned char a = 0; a < number_of_players; a++) {
        initThreads[a] = std::async(&Networking::handleInitNetworking, &networking, static_cast<unsigned char>(a + 1), filterMap(game_map, fog_of_war, a+1), ignore_timeout, &player_names[a]);
    }
    for(unsigned char a = 0; a < number_of_players; a++) {
    ...
    while(turn_number < maxTurnNumber && (std::count(result.begin(), result.end(), true) > 1 || (number_of_players == 1 && productive_squares_remaining > 0))) {
        ...
        //Count productive squares remaining for Blitz2k18 single-player game
    ...
    if (enabledReplay) {
      stats.output_filename = replayDirectory + "Replays/" + std::to_string(id) + '-' + std::to_string(seed) + ".blitz";
      try {
    output(stats.output_filename);
      }
      catch(std::runtime_error & e) {
    stats.output_filename = replayDirectory + std::to_string(id) + '-' + std::to_string(seed) + ".blitz";
    output(stats.output_filename);
      }
...
std::string Blitz2k18::getName(unsigned char playerTag) {
