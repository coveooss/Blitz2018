...
class GameMap:
    def __init__(self, size_string, map_string=None):
    ...
    def get_frame(self, map_string=None):
        "Updates the map information from the latest frame provided by the Blitz2k18 game environment."
        self.production = tuple(tuple(map(int, substring)) for substring in grouper(get_string().split(), self.width))
...
#################################################################
# Functions for communicating with the Blitz2k18 game environment  #
#################################################################
...
def translate_cardinal(direction):
    "Translate direction constants used by this Python-based bot framework to that used by the official Blitz2k18 game environment."
